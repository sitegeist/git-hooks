...
function testOutputForCommitMessageWithoutPrimaryPrefix() {
	...
	echo $OUTPUT > "$OUTPUT_DIR/commit-msg.result.txt"

	diff "$OUTPUT_DIR/commit-msg.result.txt" "$OUTPUT_DIR/commit-msg.txt"
	assertEquals "The commit-msg should output a commit-msg guide if no prefix tags are found within the commit msg." 0 $?
...
function testReturnCodeForCommitMessageWithoutPrimaryPrefix() {
	...
	assertEquals "The commit-msg hook should exit with an error code if no primary or secondary prefix was found." 1 $?
...
function testReturnCodeForCommitMessageWithInvalidPrefix() {
	...
	assertEquals "The commit-msg hook should exit with an error code if an invalid prefix was found." 1 $?

	# Invalid [TASK] prefix.
	modifyTestFile
	git commit -m "[TASK My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an error code if an incomplete prefix was found." 1 $?

	# Missing space between the prefix and the message.
	modifyTestFile
	git commit -m "[TASK]My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an error code if no space was found between the prefix and the message." 1 $?
...
function testReturnCodeForCommitMessageWithPrimaryPrefixes() {
	...
	assertEquals "The commit-msg hook should exit with an success code if the [FEATURE] prefix was found." 0 $?

	# Test the [TASK] prefix.
	modifyTestFile
	git commit -m "[TASK] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [TASK] prefix was found." 0 $?

	# Test the [BUGFIX] prefix.
	modifyTestFile
	git commit -m "[BUGFIX] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [BUGFIX] prefix was found." 0 $?

	# Test the [DOCS] prefix.
	modifyTestFile
	git commit -m "[DOCS] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [DOCS] prefix was found." 0 $?

	# Test the [CLEANUP] prefix.
	modifyTestFile
	git commit -m "[CLEANUP] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [CLEANUP] prefix was found." 0 $?
...
function testReturnCodeForCommitMessageWithInvalidSecondaryPrefixes() {
	...
	modifyTestFile
	git commit -m "[!!!] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an error code if the [!!!] prefix was found but no primary is followed directly after." 1 $?

	# Test the [WIP] prefix.
	modifyTestFile
	git commit -m "[WIP] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an error code if the [WIP] prefix was found but no primary is followed directly after." 1 $?

	# Test the an invalid secondary prefix with an valid primary one.
	modifyTestFile
	git commit -m "[FAQ][TASK] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an error code if an invalid secondary prefix and a valid primary is followed directly after." 1 $?
...
function testReturnCodeForCommitMessageWithSecondaryPrefixes() {
	...
	modifyTestFile
	git commit -m "[!!!][FEATURE] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [!!!][FEATURE] prefixes where found." 0 $?

	# Test the [!!!][TASK] prefix.
	modifyTestFile
	git commit -m "[!!!][TASK] My commit message." > /dev/null 2>&1
	assertEquals "The commit-msg hook should exit with an success code if the [!!!][TASK] prefixes where found." 0 $?
